...
export default function UserProfile() {
  ...
  async function sendData(values) {
    ...
    peticionGet(values.nombrePlato);
    ...
    if(platoExiste){
      ...
      await Axios(authOptions)
        .then(function(response) {
          alert("El plato ha sido registrado de manera correcta");       
        })
        .catch(function(error) {
          alert("Ha ocurrido un fallo. No ha sido posible realizar el registro");
        });
    }else{
      alert("No ha sido posible realizar el registro. El nombre del plato ya se encuentra en uso");
    }
  }

  var platoExiste = false;

  const peticionGet=async(nombrePlatoActual)=>{   
    platoExiste = false;
    const baseUrlGet = baseUrl+`/buscar-por-nombre/`+nombrePlatoActual;
    await Axios.get(baseUrlGet)
    .then(res => {
